...
### ۳.۵) اعتبارسنجی پیش‌دستانه و یکپارچه‌ی سطح تفکر (`resolveGemini3ThinkingLevel`)

برای جلوگیری از ارسال مقادیر نامعتبر `thinkingLevel` به Gemini 3.x (که فقط ۴ مقدار بسته `minimal`, `low`, `medium`, `high` را می‌پذیرد) و رفع خطاهای ۴۰۰ از سمت کلاینت‌هایی مانند Hermes (که مقدار `none` ارسال می‌کرد) یا اشکال غیرمنتظره از Claude Code (مانند `adaptive`)، یک تابع اعتبارسنجی مشترک در `src/nativeTranslate.js` پیاده‌سازی شد.

این تابع پیش از ارسال درخواست به Google:
۱. مقادیری مانند `"none"`, `"off"`, `"disabled"`, `"false"`, `"0"` را مستقیماً به `"minimal"` نگاشت می‌کند.
۲. مقادیر معتبر را صرف‌نظر از کوچک/بزرگ بودن حروف پاس می‌دهد.
۳. در صورت نامعتبر بودن یا ناشناخته بودن مقدار، به `default_thinking` ثبت‌شده‌ی خودِ مدل در دیتابیس رجوع می‌کند.
۴. در صورت عدم وجود هیچ مقدار معتبر، تنظیمات thinking را حذف می‌کند تا خودِ Gemini از رفتار پیش‌فرض بومی‌اش استفاده کند.

با این روش، نیاز به retry واکنش‌گرا و پیام‌های هشدار تلگرام کاملاً برطرف شد.
